...
import { courseCatalog, dashboardData } from "@/lib/mock-data"

export default function DashboardPage() {
  const continueCourse = courseCatalog.find(
    (course) => course.id === dashboardData.continueCourseId
  )!
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-base leading-7 text-[var(--text-dim)] text-pretty sm:text-lg">
                Your next session is already shaped. Stay inside the concept
                that is wobbling, then cash in the momentum while it is still
                warm.
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <h2 className="serif text-3xl font-semibold tracking-tight text-[var(--text)]">
              Tuto noticed you are weak on {dashboardData.insightTopic}.
            </h2>
            <p className="max-w-2xl text-base leading-7 text-[var(--text-dim)]">
              We slowed down the next path, added one more visual prompt, and
              kept the practice load focused on the same idea.
            </p>
          </div>
          <div className="inline-flex items-center gap-3 rounded-[var(--radius-sm)] border border-[color:color-mix(in_srgb,var(--accent)_24%,var(--border))] bg-[var(--bg)] px-4 py-3 text-sm text-[var(--text-dim)]">
            ...
            Adaptive review is shaping tomorrow already.
          </div>
        </div>
      </section>
