...
import { parseChartConfig, sanitizeSvg } from '@/lib/visualization-rendering'
...
function SvgRenderer({ svg }: { svg: string }) {
  ...
  if (sanitized.error) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900/60 dark:bg-red-950/30">
        ...
        <pre className="mt-2 whitespace-pre-wrap text-xs text-red-500">{t(sanitized.error)}</pre>
      </div>
    )
  }
  ...
  return (
    <div
      ...
      dangerouslySetInnerHTML={{ __html: sanitized.svg }}
    />
  )
}
